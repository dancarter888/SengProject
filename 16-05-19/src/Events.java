...
public class Events {
    ...
    public static String asteroidBelt(Ship ship) {
    	String text = "";
        Random rand = new Random();
        int num = rand.nextInt(2);
        if(num == 1) {
            ship.damageShields();
            text = (String.format("Asteroid belt hit while travelling to %s!\n Shields are now %d", ship.getLocation(), ship.getShields()));
